...
import { FocusWall } from './focus-wall'
...
export function FocusTimer({ onSessionEnd }: FocusTimerProps) {
  ...
  function stop() {
    ...
    setIsPaused(false)
    const end = new Date()
    const sessionFocus = focusSeconds
    setTotalFocusSeconds((prev) => prev + sessionFocus)
    onSessionEnd({
      start: startTime || new Date(),
      end,
      focus: sessionFocus,
      breakTime: Math.floor(sessionFocus / 180) * 60,
    })
    setFocusSeconds(0)
    setStartTime(null)
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <FocusWall totalFocusSeconds={totalFocusSeconds} />
    </div>
  )
}
